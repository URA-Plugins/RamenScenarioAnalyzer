...
`producer.Update(new RamenTrainingDisplayId(charaId, turn), part)` replaces only that producer's part for the specified display ID and does not touch the View. `RamenTrainingDisplay.Show(id)` combines that ID's scenario base part and producer parts, then publishes once; it returns `false` when the scenario part is not available or cancellation was requested. Same-producer updates are last-write-wins, different IDs are isolated, and registration, Update, and Dispose never publish implicitly. The public Important, Extra, training-card, and scenario-panel editors accept plain text or colored `RamenDisplaySegment` values. A failing part leaves the last successful display unchanged.
